...
function buildKeyboardWithStyle(btnsDef, modelo={}, ctx=null){
  ...
  btnsDef.forEach(b=>{
    ...
    if(b.style) btn.style=b.style; // 🟢 success 🔵 primary 🔴 danger
...
bot.command('admin', async(ctx)=>{
  ...
  await ctx.reply(`👑 𝗣𝗔𝗡𝗘𝗟 𝗗𝗘 𝗔𝗗𝗠𝗜𝗡𝗦`,{reply_markup:{inline_keyboard:[
    ...
    [{text:"🌐 𝘼𝘿𝙈𝙄𝙉 𝙒𝙀𝘽", web_app:{url:"https://bot-galeria-verified.vercel.app/admin.html"}, style:"success"}]
  ]}});
});

bot.action('panel_bienvenida', async(ctx)=>{ try{ await ctx.answerCbQuery(); }catch(e){} let c=await getConfig(); await ctx.reply(`BIENVENIDA Foto: ${c.bienvenida_media?'✅':'❌'}\nTexto: ${(c.bienvenida_texto||'').substring(0,300)}`,{reply_markup:{inline_keyboard:[[{text:"📸 Cambiar Foto", callback_data:"edit_bienvenida_foto", style:"primary"}, {text:"📝 Texto", callback_data:"edit_bienvenida_texto", style:"primary"}], [{text:"👁️ Preview", callback_data:"preview_start", style:"success"}], [{text:"⬅️ Volver", callback_data:"back_panel", style:"danger"}]]}}); });
bot.action('panel_galeria', async(ctx)=>{ try{ await ctx.answerCbQuery(); }catch(e){} let c=await getConfig(); await ctx.reply(`GALERIA Foto: ${c.galeria_media?'✅':'❌'}\nTexto: ${(c.galeria_texto||'').substring(0,300)}`,{reply_markup:{inline_keyboard:[[{text:"📸 Foto", callback_data:"edit_galeria_foto", style:"primary"}, {text:"📝 Texto", callback_data:"edit_galeria_texto", style:"primary"}], [{text:"🧩 Emoji", callback_data:"edit_galeria_emoji", style:"primary"}], [{text:"⬅️ Volver", callback_data:"back_panel", style:"danger"}]]}}); });
bot.action('panel_usuarios', async(ctx)=>{ try{ await ctx.answerCbQuery(); }catch(e){} let snap=await getDocs(collection(db,"usuarios")).catch(()=>({size:0})); await ctx.reply(`Usuarios: ${snap.size}`,{reply_markup:{inline_keyboard:[[{text:"⬅️ Volver", callback_data:"back_panel", style:"danger"}]]}}); });
bot.action('panel_plantillas', async(ctx)=>{ try{ await ctx.answerCbQuery(); }catch(e){} let c=await getConfig(); let snap=await getDocs(collection(db,"plantillas")).catch(()=>({docs:[]})); let kb=[]; snap.docs.slice(0,10).forEach(d=>{ kb.push([{text:`📄 ${d.data().nombre||d.id.slice(-6)}`, callback_data:`plantilla_use_${d.id}`, style:"primary"}, {text:"🗑️", callback_data:`plantilla_del_${d.id}`, style:"danger"}]); }); kb.push([{text:"📝 Nueva", callback_data:"edit_plantilla_texto", style:"success"}]); kb.push([{text:"⬅️ Volver", callback_data:"back_panel", style:"danger"}]); await ctx.reply(`Plantilla:\n${(c.plantilla_texto||'').substring(0,350)}`,{reply_markup:{inline_keyboard:kb}}); });
...
bot.action('edit_bienvenida_foto', async(ctx)=>{ try{ await ctx.answerCbQuery(); }catch(e){} esperando[ctx.from.id]='foto_bienvenida'; await ctx.reply("📸 Manda FOTO"); });
...
bot.action('edit_galeria_emoji', async(ctx)=>{ try{ await ctx.answerCbQuery(); }catch(e){} esperando[ctx.from.id]='emoji_galeria'; await ctx.reply("🧩 Manda emoji premium"); });
...
module.exports = async(req,res)=>{
  if(req.method==='GET') return res.status(200).send('Bot OK - CON STYLE 🟢🔵🔴 + SETFOTO');
