...
// HELPER: EDITA EL MISMO MENSAJE EN LUGAR DE MANDAR UNO NUEVO
async function editOrReply(ctx, media, texto, keyboard){
  try{
    if(ctx.callbackQuery && ctx.callbackQuery.message){
      // Si hay foto nueva, editamos la media del mismo mensaje
      if(media){
        await ctx.editMessageMedia({type:'photo', media: media, caption: texto, parse_mode:'HTML'}, {reply_markup:{inline_keyboard:keyboard}});
        return;
      }else{
    ...
    console.log("Edit fallo, mando nuevo:", e.message);
    // Si falla editar, borramos y mandamos nuevo
...
bot.action('edit_bienvenida_texto', async(ctx)=>{ await ctx.answerCbQuery(); esperando[ctx.from.id]='texto_bienvenida'; await ctx.reply("📝 Manda texto con {mencion} si quieres mención."); });
...
bot.on('text', async(ctx, next)=>{
  ...
  if(st==='texto_bienvenida'){ await setDoc(doc(db,"config","bot"),{bienvenida_texto:txt},{merge:true}); delete esperando[ctx.from.id]; return ctx.reply(`✅ Texto con {mencion} OPCIONAL guardado`); }
...
// --- LISTA SOBRE MISMO MENSAJE - SOLO NOMBRE ---
bot.action('lista', async(ctx)=>{
  ...
  snap.forEach(d=>{
    let m=d.data();
    let btn={text: `${m.perfil}`, callback_data:`ver_${d.id}`, style:["primary","success","danger"][i%3]};
    if(emojiPremium) btn.icon_custom_emoji_id=emojiPremium;
    row.push(btn);
    if(row.length===2){ keyboard.push(row); row=[]; } // 2 por fila
    i++;
  });
...
// --- VER MODELO SOBRE MISMO MENSAJE ---
bot.action(/ver_(.*)/, async(ctx)=>{
  ...
  kb.push([{text:"⬅️ Volver", callback_data:"lista", style:"primary"}, {text:"🏠 Inicio", callback_data:"inicio", style:""}]);
...
module.exports = async(req,res)=>{
  if(req.method==='GET') return res.status(200).send('Bot OK - MISMO MENSAJE + solo nombre + {mencion} opcional');
