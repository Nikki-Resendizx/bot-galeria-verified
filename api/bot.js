...
// FIX 1: evita "Firebase App already exists" en Vercel
if (!getApps().length) {
...
function buildKeyboard(btnsDef, modelo={}, ctx=null){
  let rowsMap={};
  if(!btnsDef ||!Array.isArray(btnsDef)) btnsDef = [];
  btnsDef.forEach(b=>{
    let text=replaceVars(b.text||"BOTON", modelo, ctx);
    let btn={text}; if(b.style) btn.style=b.style; if(b.icon_custom_emoji_id) btn.icon_custom_emoji_id=String(b.icon_custom_emoji_id);
    if(b.type==='web_app') btn.web_app={url: replaceVars(b.url||WEBAPP_URL, modelo, ctx)}; else if(b.type==='url') btn.url=replaceVars(b.url||"", modelo, ctx); else btn.callback_data=replaceVars(b.data||"lista", modelo, ctx);
    let row = b.row?? 0;
    if(!rowsMap[row]) rowsMap[row]=[]; rowsMap[row].push(btn);
  });
...
// FIX FOTOS - solo regresa string o file_id
...
bot.start(async(ctx)=>{
  try{
    ...
    let emojiBienvenida = c.bienvenida_emoji_premium || c.galeria_emoji_premium || "";
    let defaultKb = [
      {text:"💖 𝗩𝗘𝗥 𝗚𝗔𝗟𝗘𝗥𝗜𝗔 𝗩𝗜𝗥𝗧𝗨𝗔𝗟 💖", type:"web_app", url:WEBAPP_URL, style:"success", row:0, icon_custom_emoji_id: emojiBienvenida},
      {text:"👑 𝗩𝗘𝗥 𝗟𝗜𝗦𝗧𝗔 𝗗𝗘 𝗠𝗢𝗗𝗘𝗟𝗢𝗦 👑", type:"callback", data:"lista", style:"primary", row:1, icon_custom_emoji_id: emojiBienvenida}
    ];
    let kb=buildKeyboard(c.bienvenida_botones||defaultKb, {}, ctx);
    if(kb.length===0) kb = buildKeyboard(defaultKb, {}, ctx);
  ...
  }catch(e){ console.error("START ERROR", e); await ctx.reply("Hola 👑 Bienvenid@\nUsa /admin",{reply_markup:{inline_keyboard:[[{text:"💖 GALERIA", web_app:{url:WEBAPP_URL}}]]}}); }
});
...
bot.command('addadmin', async(ctx)=>{ if(!(await isAdmin(ctx))) return; let newId = ctx.message.text.split(' ')[1]; if(!newId) return ctx.reply("Usa: /addadmin 123456789"); let c=await getConfig(); let admins = c.admins||[]; if(!admins.includes(String(newId))) admins.push(String(newId)); await setDoc(doc(db,"config","bot"),{admins},{merge:true}); await ctx.reply(`✅ Admin ${newId} agregado`); });
...
bot.command('admins', async(ctx)=>{ if(!(await isAdmin(ctx))) return; let c=await getConfig(); await ctx.reply(`Admins ENV: ${ADMIN_IDS_ENV.join(',')}\nFirestore: ${(c.admins||[]).join(',')}`); });
bot.command('ban', async(ctx)=>{ if(!(await isAdmin(ctx))) return; let id=ctx.message.text.split(' ')[1]; if(!id) return ctx.reply("Usa: /ban 123"); await setDoc(doc(db,"usuarios", String(id)),{banned:true},{merge:true}); await ctx.reply(`🚫 Baneado ${id}`); });
...
bot.command('banned', async(ctx)=>{ if(!(await isAdmin(ctx))) return; const snap=await getDocs(collection(db,"usuarios")); let list=[]; snap.forEach(d=>{ if(d.data().banned) list.push(d.id); }); await ctx.reply(`Baneados: ${list.join(', ')||'ninguno'}`); });
...
bot.command('admin', async(ctx)=>{
  ...
  await ctx.reply(`👑 <b>𝗣𝗔𝗡𝗘𝗟 𝗗𝗘 𝗔𝗗𝗠𝗜𝗡𝗦</b> 👑`,{parse_mode:'HTML',reply_markup:{inline_keyboard:[
    ...
    [{text:"👤 𝙐𝙎𝙐𝘼𝙍𝙄𝙊𝙎", callback_data:"panel_usuarios"}, {text:"📝 𝙋𝙇𝘼𝙉𝙏𝙄𝙇𝘼𝙎", callback_data:"panel_plantillas"}],
    [{text:"👑 ADMINS/BAN", callback_data:"panel_admins"}],
    [{text:"🌐 𝘼𝘿𝙈𝙄𝙉 𝙋𝘼𝙉𝙀𝙇 𝙒𝙀𝘽", web_app:{url:"https://bot-galeria-verified.vercel.app/admin.html"}}]
  ]}});
});

bot.action('panel_bienvenida', async(ctx)=>{ await ctx.answerCbQuery(); let c=await getConfig(); await ctx.reply(`Foto: ${c.bienvenida_media?'✅':'❌'} Emoji: ${c.bienvenida_emoji_premium||'no'}\nTexto: ${(c.bienvenida_texto||'').substring(0,400)}`,{parse_mode:'HTML',reply_markup:{inline_keyboard:[[{text:"📸 Cambiar Foto", callback_data:"edit_bienvenida_foto"}, {text:"📝 Texto", callback_data:"edit_bienvenida_texto"}], [{text:"🧩 Emoji premium botones", callback_data:"edit_bienvenida_emoji"}], [{text:"👁️ Preview /start", callback_data:"preview_start"}], [{text:"⬅️ Volver", callback_data:"back_panel"}]]}}); });
...
bot.action('panel_usuarios', async(ctx)=>{ await ctx.answerCbQuery(); let snap=await getDocs(collection(db,"usuarios")).catch(()=>({size:0})); let c=await getConfig(); await ctx.reply(`👤 Usuarios: ${snap.size}\nAdmins: ${(c.admins||[]).join(', ')}\n/ban ID /unban ID /banned /addadmin ID`,{reply_markup:{inline_keyboard:[[{text:"⬅️ Volver", callback_data:"back_panel"}]]}}); });
bot.action('panel_admins', async(ctx)=>{ await ctx.answerCbQuery(); let c=await getConfig(); await ctx.reply(`👑 Admins ENV: ${ADMIN_IDS_ENV.join(',')}\nFirestore: ${(c.admins||[]).join(', ')}\n\nUsa /addadmin ID /deladmin ID /ban ID`,{reply_markup:{inline_keyboard:[[{text:"⬅️ Volver", callback_data:"back_panel"}]]}}); });
bot.action('panel_plantillas', async(ctx)=>{ await ctx.answerCbQuery(); let c=await getConfig(); let snap=await getDocs(collection(db,"plantillas")).catch(()=>({docs:[]})); let kb=[]; snap.docs.slice(0,10).forEach(p=>{ kb.push([{text:`📄 ${p.data().nombre||p.id.slice(-6)}`, callback_data:`plantilla_use_${p.id}`}, {text:"🗑️", callback_data:`plantilla_del_${p.id}`}]); }); kb.push([{text:"📝 Nueva (desde bot)", callback_data:"edit_plantilla_texto"}]); kb.push([{text:"⬅️ Volver", callback_data:"back_panel"}]); await ctx.reply(`📝 ACTUAL:\n${(c.plantilla_texto||'').substring(0,400)}\n\nGuardadas: ${snap.docs.length}`,{parse_mode:'HTML', reply_markup:{inline_keyboard:kb}}); });
...
bot.action('back_panel', async(ctx)=>{ await ctx.answerCbQuery(); return ctx.reply("/admin"); });
...
bot.action('edit_bienvenida_texto', async(ctx)=>{ await ctx.answerCbQuery(); esperando[ctx.from.id]='texto_bienvenida'; await ctx.reply("📝 Manda texto con {mencion} + premium"); });
bot.action('edit_bienvenida_emoji', async(ctx)=>{ await ctx.answerCbQuery(); esperando[ctx.from.id]='emoji_bienvenida'; await ctx.reply("🧩 Manda UN emoji premium para botones BIENVENIDA"); });
...
bot.action('edit_galeria_emoji', async(ctx)=>{ await ctx.answerCbQuery(); esperando[ctx.from.id]='emoji_galeria'; await ctx.reply("🧩 Manda UN emoji premium para botones GALERIA/PERFIL"); });
bot.action('edit_plantilla_texto', async(ctx)=>{ await ctx.answerCbQuery(); esperando[ctx.from.id]='texto_plantilla'; await ctx.reply("📝 Manda plantilla con {mencion} {perfil} + premium\nLuego te pediré NOMBRE para guardarla múltiple"); });

// FIX 2: solo foto, sin next() que rompia
...
// FIX 3: detectar ID premium automáticamente
bot.on('message', async(ctx, next)=>{
  const ents = ctx.message.entities||[]; const custom = ents.find(e=>e.type==='custom_emoji');
  if(custom && await isAdmin(ctx)){
    const st = esperando[ctx.from.id];
    if(st==='emoji_bienvenida' || st==='emoji_galeria'){
      // deja que lo maneje el handler de text
    } else {
      await ctx.reply(`🧩 ID premium detectado: <code>${custom.custom_emoji_id}</code>`,{parse_mode:'HTML'});
    }
  }
  if(next) await next();
});
...
bot.on('text', async(ctx)=>{
  ...
  if(st==='texto_bienvenida'){ await setDoc(doc(db,"config","bot"),{bienvenida_texto:htmlText, bienvenida_premium_ids:premiumIds},{merge:true}); delete esperando[ctx.from.id]; return ctx.reply(`✅ Bienvenida guardada`); }
  if(st==='emoji_bienvenida'){ await setDoc(doc(db,"config","bot"),{bienvenida_emoji_premium:String(first), galeria_emoji_premium:String(first)},{merge:true}); delete esperando[ctx.from.id]; return ctx.reply(`✅ Emoji bienvenida: ${first}`); }
  if(st==='texto_galeria'){ await setDoc(doc(db,"config","bot"),{galeria_texto:htmlText},{merge:true}); delete esperando[ctx.from.id]; return ctx.reply(`✅ Galería guardada`); }
  if(st==='emoji_galeria'){ await setDoc(doc(db,"config","bot"),{galeria_emoji_premium:String(first)},{merge:true}); delete esperando[ctx.from.id]; return ctx.reply(`✅ Emoji premium botones: ${first}`); }
  if(st==='texto_plantilla'){ plantillaTemp[ctx.from.id]={texto:htmlText, premium_ids:premiumIds, premium_log:premiumLog}; esperando[ctx.from.id]='nombre_plantilla'; return ctx.reply(`✅ Plantilla recibida\nAhora manda el NOMBRE para guardarla en plantillas/{id}\nEj: Plantilla Rosa 1`); }
  if(st==='nombre_plantilla'){ const temp=plantillaTemp[ctx.from.id]; if(!temp) { delete esperando[ctx.from.id]; return ctx.reply("Error, /admin > Plantillas otra vez"); } const nombre=txt.trim().slice(0,40); const newId=Date.now().toString(); await setDoc(doc(db,"plantillas",newId),{nombre, texto:temp.texto, premium_ids:temp.premium_ids, premium_log:temp.premium_log, fecha:new Date().toISOString()}); await setDoc(doc(db,"config","bot"),{plantilla_texto:temp.texto},{merge:true}); delete esperando[ctx.from.id]; delete plantillaTemp[ctx.from.id]; return ctx.reply(`✅ Plantilla "${nombre}" guardada como ${newId} y activada\nYa puedes guardar infinitas desde el bot`); }
});
...
bot.action('lista', async(ctx)=>{
  try{
    ...
    snap.forEach(d=>{
      let m=d.data(); let nombre=(m.perfil||d.id).replace(/@/g,'').trim();
      ...
      if(c.bienvenida_emoji_premium) btn.icon_custom_emoji_id=String(c.bienvenida_emoji_premium);
      row.push(btn); idx++; if(row.length===2){ keyboard.push(row); row=[]; }
    });
    if(row.length>0) keyboard.push(row);
    keyboard.push([{text:"𝗩𝗘𝗥 𝗚𝗔𝗟𝗘𝗥𝗜𝗔 𝗩𝗜𝗥𝗧𝗨𝗔𝗟 💖", web_app:{url:WEBAPP_URL}, style:"success", icon_custom_emoji_id: c.galeria_emoji_premium||c.bienvenida_emoji_premium||""}]);
...
bot.action(/ver_(.*)/, async(ctx)=>{
  try{
    ...
    let eObj = emojiBtn? {icon_custom_emoji_id:String(emojiBtn)} : {};
    let kb=[
      [{text:"💖 𝗩𝗘𝗥 𝗣𝗘𝗥𝗙𝗜𝗟 𝗖𝗢𝗠𝗣𝗟𝗘𝗧𝗢 💖", web_app:{url:`${WEBAPP_URL}?m=${m.id}`}, style:"primary",...eObj}],
    ...
    if(media){
      ...
      catch(e){ try{ await ctx.replyWithPhoto({url: media},{caption, parse_mode:'HTML', reply_markup:{inline_keyboard:kb}}); return; }catch(e2){ await ctx.reply(caption,{parse_mode:'HTML', reply_markup:{inline_keyboard:kb}}); return; } }
    }
    await ctx.reply(caption,{parse_mode:'HTML', reply_markup:{inline_keyboard:kb}});
  }catch(e){ console.error("ver_ error", e); }
});

bot.action('inicio', async(ctx)=>{ try{ await ctx.answerCbQuery(); let c=await getConfig(); let texto=replaceVars(c.bienvenida_texto||"Hola {mencion} 👑", {}, ctx); let kb=buildKeyboard(c.bienvenida_botones||[{text:"💖 ABRIR GALERÍA", type:"web_app", url:WEBAPP_URL, style:"success", row:0},{text:"📋 VER LISTA", type:"callback", data:"lista", style:"primary", row:1}], {}, ctx); let media=c.bienvenida_media||c.bienvenida_media_file_id; try{ await ctx.deleteMessage(); }catch(e){} if(media){ try{ await ctx.replyWithPhoto(media,{caption:texto, parse_mode:'HTML', reply_markup:{inline_keyboard:kb}}); return; }catch(e){} } await ctx.reply(texto,{parse_mode:'HTML', reply_markup:{inline_keyboard:kb}}); }catch(e){} });
bot.action(/voto_(bueno|malo)_(.*)/, async(ctx)=>{ try{ await ctx.answerCbQuery("Voto guardado ✅"); let tipo=ctx.match[1]; let id=ctx.match[2]; let ref=doc(db,"modelos",id); if(tipo==='bueno') await updateDoc(ref,{votosBueno:increment(1)}).catch(async()=>{ await setDoc(ref,{votosBueno:1},{merge:true}); }); else await updateDoc(ref,{votosMalo:increment(1)}).catch(async()=>{ await setDoc(ref,{votosMalo:1},{merge:true}); }); let snap=await getDoc(ref); let m={id:snap.id,...snap.data()}; let c=await getConfig(); let caption=replaceVars(c.plantilla_texto||"👑 {perfil} 👑\nVotos: {votos}", m, ctx); let canalFree = m.canalFree || "https://t.me/"; let contacto = m.contacto || (m.username? `https://t.me/${m.username}` : "https://t.me/"); let emojiBtn = c.galeria_emoji_premium || ""; let kb=[[{text:"💖 𝗩𝗘𝗥 𝗣𝗘𝗥𝗙𝗜𝗟 𝗖𝗢𝗠𝗣𝗟𝗘𝗧𝗢 💖", web_app:{url:`${WEBAPP_URL}?m=${m.id}`}, style:"primary", icon_custom_emoji_id: emojiBtn}], [{text:`👍 Bueno ${m.votosBueno||0}`, callback_data:`voto_bueno_${m.id}`, style:"success"}, {text:`👎 Malo ${m.votosMalo||0}`, callback_data:`voto_malo_${m.id}`, style:"danger"}], [{text:"💎 𝗖𝗔𝗡𝗔𝗟 𝗙𝗥𝗘𝗘", url:canalFree, style:"primary"}, {text:"💬 𝗖𝗢𝗡𝗧𝗔𝗖𝗧𝗔𝗥", url:contacto, style:"primary"}], [{text:"👈🏻 VOLVER", callback_data:"lista", style:"danger"}, {text:"👑 INICIO", callback_data:"inicio", style:"danger"}]]; let media = getMediaModelo(m); try{ await ctx.deleteMessage(); }catch(e){} if(media){ try{ await ctx.replyWithPhoto(media,{caption, parse_mode:'HTML', reply_markup:{inline_keyboard:kb}}); return; }catch(e){} } await ctx.reply(caption,{parse_mode:'HTML', reply_markup:{inline_keyboard:kb}}); }catch(e){} });
bot.action('preview_start', async(ctx)=>{ await ctx.answerCbQuery(); let c=await getConfig(); let texto=replaceVars(c.bienvenida_texto||"Hola {mencion}", {}, ctx); let kb=buildKeyboard(c.bienvenida_botones||[], {}, ctx); let media=c.bienvenida_media||c.bienvenida_media_file_id||c.bienvenida_media_url; try{ await ctx.deleteMessage(); }catch(e){} if(media){ try{ await ctx.replyWithPhoto(media,{caption:texto, parse_mode:'HTML', reply_markup:{inline_keyboard:kb}}); return; }catch(e){} } await ctx.reply(texto,{parse_mode:'HTML', reply_markup:{inline_keyboard:kb}}); });

module.exports = async(req,res)=>{
  if(req.method==='GET') return res.status(200).send('Bot OK - CORREGIDO START + FOTO + BAN + PLANTILLAS MULTI + EMOJI');
